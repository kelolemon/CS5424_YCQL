...
func SetNewWarehouseYTD(warehouseID int32, newWarehouseYTD float64) (err error) {
	if err := dao.Session.Query(`UPDATE Warehouse SET W_YTD = ? WHERE W_ID = ?`, newWarehouseYTD, warehouseID).Exec(); err != nil {
...
func GetWarehouseInfo(warehouseID int32) (warehouseInfo common.Warehouse, err error) {
	if err := dao.Session.Query(`SELECT * FROM Warehouse WHERE W_ID = ?`, warehouseID).Scan(&warehouseInfo); err != nil {
