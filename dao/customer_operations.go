...
func SetNewCustomerPaymentInfo(CustomerID int32, newCustomerBalance float64, newCustomerYTD float64, newCustomerPaymentCnt int32) (err error) {
	if err := dao.Session.Query(`UPDATE Customer SET C_BALANCE = ?, C_YTD_PAYMENT = ?, C_PAYMENT_CNT = newCustomerPaymentCnt WHERE C_ID = ?`, newCustomerBalance, newCustomerYTD, newCustomerPaymentCnt, CustomerID); err != nil {
...
func GetCustomerInfo(customerID int32) (customerInfo common.Customer, err error) {
	if err := dao.Session.Query(`SELECT * FROM Customer WHERE C_ID = ?`, customerID).Scan(&customerInfo); err != nil {
